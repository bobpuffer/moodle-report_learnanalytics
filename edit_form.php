...
class report_learnanalytics_edit_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'general', get_string('pluginname', 'report_learnanalytics'));

        $mform->addElement('header', 'weightings', get_string('weighting', 'report_learnanalytics'));
        $mform->addElement('static', 'weightings_desc', get_string('indicator', 'report_learnanalytics'));
    ...
    public function validation($data, $files) {
        ...
        foreach ($indicators as $indicator => $path) {
            ...
            if (isset($data[$key]) && (!is_numeric($data[$key]) || $data[$key] > 100 || $data[$key] < 0)) {
                $errors["weight_group_$indicator"] = get_string('weightingmustbenumeric', 'report_learnanalytics');
                continue;
            }
            if (isset($data[$key])) {
                $sum += $data[$key];
        ...
        if ($sum != 100) {
            $errors['weightings_desc'] = get_string('weightingsumtoonehundred', 'report_learnanalytics');
        }
