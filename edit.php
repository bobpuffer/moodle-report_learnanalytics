...
$url = new moodle_url('/report/learnanalytics/edit.php', array('id' => $id));
$reporturl = new moodle_url('/report/learnanalytics/index.php', array('id' => $id));
$PAGE->set_url($url);
...
require_capability('report/learnanalytics:manage', $context);

$strpluginname = get_string('pluginname', 'report_learnanalytics');
...
$PAGE->navbar->add(get_string('updatesettings', 'report_learnanalytics'), $url);
...
if ($mform->is_cancelled()) {
    redirect(new moodle_url('/report/learnanalytics/index.php', array('id' => $id)));
    ...
    report_learnanalytics_update_indicator($id, $weights, $configdata);
}

// Get current values and populate form.
$data = array();
if ($indicators = $DB->get_records('report_learnanalytics', array('course' => $id))) {
...
add_to_log($course->id, 'course', 'report learnanalytics edit', "report/learnanalytics/edit.php?id=$id", $course->id);
